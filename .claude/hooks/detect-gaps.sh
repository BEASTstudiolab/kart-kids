...
# --- Check 0: Fresh project detection (suggests /game-start) ---
FRESH_PROJECT=true
...
if [ "$FRESH_PROJECT" = true ]; then
  ...
  echo "   This looks like a fresh start! Run: /game-start"
  echo ""
  echo "💡 To get a comprehensive project analysis, run: /game-project-stage-detect"
  echo "==================================="
  exit 0
fi
...
if [ "$SRC_FILES" -gt 50 ] && [ "$DESIGN_FILES" -lt 5 ]; then
  ...
  echo "    Suggested action: /game-reverse-document design src/[system]"
  echo "    Or run: /game-project-stage-detect to get full analysis"
fi
...
if [ -d "prototypes" ]; then
  ...
  if [ -n "$PROTOTYPE_DIRS" ]; then
    ...
    if [ ${#UNDOCUMENTED_PROTOS[@]} -gt 0 ]; then
      ...
      echo "    Suggested action: /game-reverse-document concept prototypes/[name]"
    fi
  fi
fi
...
if [ -d "src/core" ] || [ -d "src/engine" ]; then
  if [ ! -d "docs/architecture" ]; then
    ...
    echo "    Suggested action: Create docs/architecture/ and run /game-architecture-decision"
  else
    ...
    if [ "$ADR_COUNT" -lt 3 ]; then
      ...
      echo "    Suggested action: /game-reverse-document architecture src/core/[system]"
    fi
  fi
fi
    ...
    while IFS= read -r system_dir; do
      ...
      if [ "$file_count" -ge 5 ]; then
        ...
        if [ ! -f "$design_doc_1" ] && [ ! -f "$design_doc_2" ]; then
          ...
          echo "    Suggested action: /game-reverse-document design src/gameplay/$system_name"
        fi
      fi
...
if [ "$SRC_FILES" -gt 100 ]; then
  ...
  if [ ! -d "production/sprints" ] && [ ! -d "production/milestones" ]; then
    ...
    echo "    Suggested action: /game-sprint-plan or create production/ directory"
  fi
fi
