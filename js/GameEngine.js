...
export function createGameEngine( canvasContainer ) {
	...
	function _getDebugAIState() {
		...
		return ( _aiManager._racers || [] ).map( ( ai, index ) => {
			...
			profileSeed: ai.profileSeed ?? null,
			style: ai.styleSummary || null,
			...
			effectiveTurnSeverity: controllerDebug ? Number( ( controllerDebug.effectiveTurnSeverity ?? controllerDebug.turnSeverity ?? 0 ).toFixed( 3 ) ) : null,
			trafficOccupancy: controllerDebug ? Number( ( controllerDebug.trafficOccupancy ?? 0 ).toFixed( 3 ) ) : null,
			spacingPressure: controllerDebug ? Number( ( controllerDebug.spacingPressure ?? 0 ).toFixed( 3 ) ) : null,
			overtakeActive: !! controllerDebug?.overtakeActive,
			overtakeDirection: controllerDebug?.overtakeDirection ?? 0,
			overtakeTargetId: controllerDebug?.overtakeTargetId ?? null,
			launchActive: !! controllerDebug?.launchActive,
			launchHolding: !! controllerDebug?.launchHolding,
			launchLaneBias: controllerDebug ? Number( ( controllerDebug.launchLaneBias ?? 0 ).toFixed( 3 ) ) : null,
			launchElapsed: controllerDebug ? Number( ( controllerDebug.launchElapsed ?? 0 ).toFixed( 3 ) ) : null,
			wallEscapeFactor: controllerDebug ? Number( ( controllerDebug.wallEscapeFactor ?? 0 ).toFixed( 3 ) ) : null,
			mistakeActive: !! controllerDebug?.mistakeActive,
			mistakeType: controllerDebug?.mistakeType ?? null,
