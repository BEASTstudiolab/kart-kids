...
export class Vehicle {
	...
	raycastGround( dt ) {
		...
		for ( let i = 0; i < 4; i ++ ) {
			...
			if ( this._rayCollector.hit.status === CastRayStatus.COLLIDING ) {
				...
				// not road surface.
				const nearWall = Math.max( this._wallProximityLeft, this._wallProximityRight ) > 0.3;
				const isWallTop = nearWall && ( hitY > this.groundHeight + 0.3 );
		...
		if ( anyHit ) {
			...
			const dx = this.vehPos.x - cx;
			const dz = this.vehPos.z - cz;
	...
	raycastWallProximity() {
		...
		const ZONE = this.debug.curbDragZone;
		const origin = [ this.vehPos.x, this._vehicleY + 0.8, this.vehPos.z ];
		...
		for ( let i = 0; i < 2; i ++ ) {
			...
			castRay( this.physicsWorld, this._rayCollector, this._raySettings,
			...
			if ( i === 0 ) this._wallProximityLeft = proximity;
			else this._wallProximityRight = proximity;
	...
	updateRemote( dt ) {
		...
		this.vehPos.copy( this._renderPos );
		this.vehVel.set( this._targetVel[ 0 ], this._targetVel[ 1 ], this._targetVel[ 2 ] );
