/**
 * TrackBrowser — Reusable track browse/select component.
 *
 * Renders a detail panel (left) + carousel rows (right) with minimap
 * thumbnails for official and user-created tracks. Cards can be clicked
 * to select a track.
 *
 * Used by TracksPanel (workshop mode) and RaceSetupPanel (race mode).
 *
 * @example
 *   const browser = new TrackBrowser( container, {
 *       onTrackSelected: ( id ) => console.log( 'Selected', id ),
...
export class TrackBrowser {
	 ...
	 * @param {Function}    [opts.onTrackSelected]   Callback fired with trackId on selection.
	 * @param {boolean}     [opts.showManageActions] Show delete/share/edit icons on user cards.
	 */
	constructor( container, opts = {} ) {
		...
		/** @type {Function|null} */
		this._onTrackSelected = opts.onTrackSelected || null;

		/** @type {boolean} */
		this._showManageActions = !! opts.showManageActions;
	...
	static _injectCSS() {
		...
		if ( TrackBrowser._cssInjected ) return;
		TrackBrowser._cssInjected = true;

		const style = document.createElement( 'style' );
		style.textContent = `

			/* ===================================================
			   TrackBrowser grid — detail + carousels
			   =================================================== */
	...
	_build() {
		...
		root.className = 'kk-tracks__grid';
		...
		root.appendChild( this._detailPanel );
		...
		this._myTracksSection = myTracksSection;
	...
	_buildUserCard( track, selectedId ) {
		...
		// Bottom area: action icon buttons (only when showManageActions is true).
		if ( this._showManageActions ) {
			...
			shareBtn.addEventListener( 'click', ( e ) => {
				...
				e.stopPropagation();
				this._onManageAction?.( 'share', track );
			...
			editBtn.addEventListener( 'click', ( e ) => {
				...
				e.stopPropagation();
				this._onManageAction?.( 'edit', track );
			...
			deleteBtn.addEventListener( 'click', ( e ) => {
				...
				e.stopPropagation();
				this._onManageAction?.( 'delete', track );
	...
	/**
	 * Select a track and notify via callback.
	 *
	 * @param {string} trackId  Registry id for built-in, 'user:' + name for user tracks.
	 */
	_selectTrack( trackId ) {

		if ( this._onTrackSelected ) {

			this._onTrackSelected( trackId );
	...
	/**
	 * Set a handler for manage actions (share/edit/delete).
	 * Called with ( action, track ) where action is 'share' | 'edit' | 'delete'.
	 *
	 * @param {Function} handler
	 */
	setManageActionHandler( handler ) {

		this._onManageAction = handler;
	...
	/**
	 * Called when the browser becomes visible.
	...
	/**
	 * Called when the browser becomes hidden.
	 */
	hide() {

		// No teardown needed — DOM persists.

	}

	/**
	 * Re-render all tracks and the detail panel.
	 */
	refresh() {
	...
	getSelectedTrackId() {
		...
		return this._settings.getSelectedTrackId();

	}

	/**
	 * Return the root element for external DOM insertion.
	 *
	...
	/**
	 * Return the my-tracks carousel section for appending extra cards.
	 *
	 * @returns {HTMLElement|null}
	 */
	getMyTracksRow() {

		return this._myTracksRow;
...
TrackBrowser._cssInjected = false;
