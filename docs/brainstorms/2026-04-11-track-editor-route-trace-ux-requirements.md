...
- R2. The auto-enabled route path overlay must only be forced on for the lifetime of the active route trace session. When route trace ends, the editor restores the user's previous route-path toggle state.
...
- R4. While route trace is active, the viewport must show playback controls for `Pause` and `Play`.
- R5. `Pause` freezes the chase camera at its current route position without exiting route trace.
- R6. `Play` resumes the chase camera from the paused position instead of restarting the route.
- R7. Disabling route trace still stops playback entirely and dismisses the playback controls.
- R8. Route trace camera movement should follow the route progression without the extra automatic orbit spin currently applied during chase updates.

**Compass Accuracy**

- R9. The editor compass must reflect the current camera heading so north/east/south/west remain truthful when the camera orbit changes.
- R10. The compass keeps its existing viewport placement and role as a lightweight orientation aid; this pass corrects behavior rather than redesigning it into a larger widget.
...
- [Affects R4-R7][Technical] Should the playback buttons live inside `track-editor.html` as persistent markup or be created by `EditorApp` only while route trace is active?
- [Affects R2][Technical] If debug mode itself was previously off, should route trace restore both `debugEnabled` and `routePath`, or only the route-path toggle and leave debug mode on if the user enabled it manually during playback?
- [Affects R9][Technical] Should the compass rotate the ring, the directional labels, or a dedicated needle so the result stays legible with minimal CSS churn?
