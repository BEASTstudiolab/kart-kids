...
const MAX_ITERATIONS_PLAN = parseEnvInt("MAX_ITERATIONS_PLAN", 10);
const MAX_ITERATIONS_WORK = parseEnvInt("MAX_ITERATIONS_WORK", 100);
const MAX_ITERATIONS_REVIEW = parseEnvInt("MAX_ITERATIONS_REVIEW", 10);
const MAX_REVIEW_ROUNDS = parseEnvInt("MAX_REVIEW_ROUNDS", 3);
const MIN_TASK_BUDGET = parseEnvInt("MIN_TASK_BUDGET", 8);
...
const TIMEOUT_PLAN = parseEnvInt("TIMEOUT_PLAN", 900);
const TIMEOUT_WORK = parseEnvInt("TIMEOUT_WORK", 1200);
const TIMEOUT_REVIEW = parseEnvInt("TIMEOUT_REVIEW", 600);
...
// Control file path (polled at task boundaries for skip/stop/pause/resume)
const CONTROL_FILE_PATH = ".sandcastle/control";
const PID_FILE_PATH = ".sandcastle/run.pid";

// XML retry cap
const MAX_XML_RETRIES = parseEnvInt("MAX_XML_RETRIES", 5);
...
// TaskOutcome and TaskLogEntry are imported from @ai-hero/sandcastle (RunLog module)
...
/** Read and consume the control file. Returns the command string or null. */
function checkControlFile(): string | null {
  try {
    const content = readFileSync(CONTROL_FILE_PATH, "utf-8").trim();
    try {
      unlinkSync(CONTROL_FILE_PATH);
    } catch {
      // file may have been removed between read and unlink
    }
    return content || null;
  } catch (err: unknown) {
    if (
    ...
    console.warn("Error reading control file:", err);
    return null;
  }
}

/** Write PID file and register cleanup handlers. */
function setupPidFile(): void {
  try {
...
const GIT_TIMEOUT = parseEnvInt("GIT_TIMEOUT", 60000);
  ...
  while (totalIterations < taskBudget) {
    ...
    try {
      result = await sandbox.run({
        ...
        maxIterations: MAX_ITERATIONS_REVIEW,
    ...
    if (!review || !validateReview(review)) {
      ...
      if (roundsWithoutImprovement >= MAX_REVIEW_ROUNDS) {
        console.log(
...
async function executeTask(
  ...
  try {
    ...
    if (task.size !== "trivial") {
      ...
      for (
        ...
        const planResult = await sandbox.run({
          ...
          maxIterations: MAX_ITERATIONS_PLAN,
          idleTimeoutSeconds: TIMEOUT_PLAN,
        });
        iterations++;
        remainingBudget--;
    ...
    const workResult = await sandbox.run({
      ...
      maxIterations: MAX_ITERATIONS_WORK,
...
const MIN_DISK_MB = parseEnvInt("MIN_DISK_MB", 2000);
...
async function main(): Promise<void> {
  ...
  for (const task of tasks) {
    ...
    // Control file check (skip / stop / pause at task boundaries)
    const control = checkControlFile();
    if (control === "stop") {
      console.log("Stopped by user.");
      break;
    }
    if (control === "skip") {
      console.log(`Skipping task: ${task.title}`);
      continue;
    }
    if (control === "pause") {
      console.log("Paused. Write 'resume' to .sandcastle/control to continue.");
      while (true) {
        await sleep(2000);
        const resume = checkControlFile();
        if (resume === "resume") {
          console.log("Resumed.");
          break;
        }
